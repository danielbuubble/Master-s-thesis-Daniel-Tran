...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/MUAP_superposition.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/raw_semg_illustration.pdf}
...
Ground loops become a direct measurement concern when two instruments are connected to the same subject~\cite{webster_medical_2020}. Consider the case where one instrument measures a biopotential and uses a reference electrode to ground the subject, while a second instrument injects a small current into the subject to measure its impedance. If the two instruments share a common ground, the injected current finds a second return path through the first instrument's reference electrode instead of returning through the impedance-measurement path~\cite{huhta_60-hz_1973}. The diverted current reduces the current through the measurement path, corrupting the impedance reading. The biopotential measurement can also be affected because the diverted current induces a voltage drop across any impedance in the shared ground path.
