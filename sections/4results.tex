...
%
% V2 build state (Apr 2026):
%   25 Main plots saved to Figures/results/.
%   Q7 = 5 plots including new per-session signal traces (P5).
%   Pooling convention locked: Method A (pool per-epoch arrays across S3-5)
%   for Q5/Q6 front-end claims; Method B (per-session aggregate, then
%   across-session SD, n=3) for Q7 reproducibility.
...
%     sample standard deviations (ddof = 1). Pooling across Sessions 3-5
%     uses Method A (pool per-epoch arrays) for Q5/Q6 metrics; Method B
%     (per-session scalar then across-session SD, n=3) for Q7
%     reproducibility metrics. SD bands on multi-epoch overlay plots are
%     a fixed neutral grey (#808080) regardless of condition color.
%     Interpretation of the results appears in Chapter 5.
% -----------------------------------------------------------------------------
...
% noise floor, contraction RMS, SNR, and mains harmonic power within
% the 10-500 Hz design bandwidth.
% Cross-references:
...
The post-isolation noise floor measured across the pre-cue window of the \num{119} contamination-filtered epochs from Session~1 was $\num{21.70} \pm \num{0.99}$\,\si{\micro\volt} \gls{rms}. Contraction-window \gls{rms} was $\num{32.17} \pm \num{4.98}$\,\si{\micro\volt}, and the per-epoch \gls{snr} was $\num{3.32} \pm \num{1.34}$\,\si{\decibel}. Figure~\ref{fig:q1-emg-overlay} shows the envelope mean and $\pm 1$ SD band, with the per-epoch traces overlaid in light red.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q1_emg_overlay_post.pdf}
  \caption{Post-isolation \gls{emg} envelope across the \num{119} contamination-filtered epochs of Session~1. The solid line shows the across-epoch mean. The grey band shows $\pm 1$ SD. Pre-cue and contraction-window envelope SDs are annotated. Light red traces show individual epochs.}
  \label{fig:q1-emg-overlay}
\end{figure}

Figure~\ref{fig:q1-emg-psd} shows the post-isolation \gls{psd}, with the analog \gls{lpf} cutoff at \SI{530}{\hertz} marked. Integrated band powers at the \SI{50}{\hertz}, \SI{100}{\hertz}, and \SI{150}{\hertz} mains harmonic locations are annotated on the figure. Table~\ref{tab:q1-emg-metrics} summarizes the per-epoch metrics.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q1_emg_psd_post.pdf}
  \caption{Post-isolation \gls{emg} \gls{psd}, Session~1. A dashed vertical line marks the analog \gls{lpf} cutoff at \SI{530}{\hertz}. Integrated band powers at the \SI{50}{\hertz}, \SI{100}{\hertz}, and \SI{150}{\hertz} mains harmonics are annotated.}
...
\begin{table}[H]
  ...
  \begin{tabular}{l c l}
    ...
    \addlinespace
    Harmonic power, \SI{50}{\hertz}             & \num{34.95}                  & \si{\micro\volt\squared}\textsuperscript{\textdagger} \\
    Harmonic power, \SI{100}{\hertz}            & \num{4.58}                   & \si{\micro\volt\squared}\textsuperscript{\textdagger} \\
    Harmonic power, \SI{150}{\hertz}            & \num{6.54}                   & \si{\micro\volt\squared}\textsuperscript{\textdagger} \\
    \bottomrule
  \end{tabular}
  \\[0.5em]
  {\footnotesize \textsuperscript{*}\gls{snr} computed per epoch as $20\log_{10}(\mathrm{contraction\,RMS} / \mathrm{noise\,floor})$. \textsuperscript{\textdagger}Pooled Welch estimate; no per-epoch SD reported.}
\end{table}
...
%
% Discussion (DO NOT include here):
%   Two-SNR-definitions interpretation (Thread H): §5.3.
%   1/f-like spectral character of σ_session: §5.3.
\subsection{Q2 - EDA Front-End Characterization}
\label{sec:results-q2}

Figure~\ref{fig:q2-eda-representative} shows a representative single epoch from Session~2, recorded simultaneously on the pre-isolation and post-isolation channels. \gls{scr} responses to the deep-inhalation cue were visible on both channels within the \SIrange{1}{6}{\second} detection window. Section~\ref{sec:results-q3} reports the quantitative comparison of pre- and post-isolation amplitude, frequency content, and waveform morphology.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q2_eda_representative_epoch_pre_post.pdf}
  \caption{Representative single epoch from Session~2 (epoch \num{30} of \num{30}), shown on the pre-isolation channel (left) and post-isolation channel (right). Dashed vertical line marks cue onset. Shaded region marks the \SIrange{1}{6}{\second} \gls{scr} detection window.}
  \label{fig:q2-eda-representative}
\end{figure}

The post-isolation \gls{scl} measured across the \num{30} epochs of Session~2 was $\num{7.714} \pm \num{0.398}$\,\si{\micro\siemens}. Mean \gls{scr} amplitude was $\num{0.588} \pm \num{0.158}$\,\si{\micro\siemens}, and rise time was $\num{3.751} \pm \num{0.776}$\,\si{\second}. Detection rate was \SI{100}{\percent} (\num{30}/\num{30} epochs). Figure~\ref{fig:q2-eda-overlay} shows the across-epoch mean conductance and $\pm 1$ SD band, with the per-epoch traces overlaid in light blue.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q2_eda_overlay_post.pdf}
  \caption{Post-isolation \gls{eda} signal across the \num{30} epochs of Session~2. The solid line shows the across-epoch mean. The grey band shows $\pm 1$ SD. Pre-cue and response-window SDs are annotated. Light blue traces show individual epochs.}
  \label{fig:q2-eda-overlay}
\end{figure}

$\sigma_{\text{session}}$ describes long-timescale baseline drift, and $\sigma_{\text{local}}$ describes within-epoch baseline variability (Section~\ref{sec:eda_processing}). For Session~2, $\sigma_{\text{session}}$ was \SI{0.549}{\micro\siemens} and $\sigma_{\text{local}}$ was $\num{0.193} \pm \num{0.021}$\,\si{\micro\siemens}. \gls{snr}$_{\text{circuit}}$ was \SI{0.60}{\decibel} (using $\sigma_{\text{session}}$), and \gls{snr}$_{\text{local}}$ was \SI{9.66}{\decibel} (using $\sigma_{\text{local}}$). Figure~\ref{fig:q2-eda-psd} shows the post-isolation \gls{psd}, with the analog \gls{lpf} cutoff at \SI{1.5}{\hertz} marked. Table~\ref{tab:q2-eda-metrics} summarizes the per-epoch metrics.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q2_eda_psd_post.pdf}
  \caption{Post-isolation \gls{eda} \gls{psd}, Session~2. A dashed vertical line marks the analog \gls{lpf} cutoff at \SI{1.5}{\hertz}.}
...
%
% Discussion (DO NOT include here):
%   σ_iso noise model (Thread A): §5.3.
%   Two-SNR-definitions asymmetry (Thread H): §5.3.
%   EMG amplitude preservation noise inflation correction (Thread I): §5.3.
%   EDA Pearson r distributional shape / 4-of-30 low-r outliers (Thread G): §5.3.
 
\subsection{Q3 - Isolation Stage Characterization}
\label{sec:results-q3}

The post-isolation \gls{emg} characterization is reported in Section~\ref{sec:results-q1}. This section reports the pre- versus post-isolation comparison from the same Session~1 data ($n = \num{119}$ contamination-filtered epochs). Amplitude preservation across the isolation stage was $+\SI{2.56}{\decibel}$, and the per-epoch Pearson correlation between the gain-corrected pre- and post-isolation channels was $\num{0.542} \pm \num{0.079}$. \gls{snr} was $\num{10.80} \pm \num{2.63}$\,\si{\decibel} pre-isolation and $\num{3.32} \pm \num{1.34}$\,\si{\decibel} post-isolation, a drop of $\SI{-7.48}{\decibel}$ (computed as post minus pre). Figure~\ref{fig:q3-emg-preservation} shows the across-epoch envelope mean and the \gls{psd} for both channels, and the per-epoch Pearson distribution is shown in the left panel of Figure~\ref{fig:q3-pearson-r}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q3_isolation_emg_preservation.pdf}
  \caption{\gls{emg} amplitude and frequency-content preservation across the isolation stage, Session~1 ($n = \num{119}$). Left: across-epoch envelope mean for the pre-isolation (light red) and post-isolation (dark red) channels. Right: \gls{psd} for both channels on log-log axes.}
  \label{fig:q3-emg-preservation}
\end{figure}

The post-isolation \gls{eda} characterization is reported in Section~\ref{sec:results-q2}. This section reports the pre- versus post-isolation comparison from the same Session~2 data ($n = \num{30}$ epochs). Amplitude preservation across the isolation stage was $+\SI{1.23}{\decibel}$, and the per-epoch Pearson correlation was $\num{0.713} \pm \num{0.162}$. \gls{scr} detection rate was \SI{100}{\percent} on both channels. \gls{snr}$_{\text{circuit}}$ was \SI{0.28}{\decibel} pre-isolation and \SI{0.60}{\decibel} post-isolation, a change of $+\SI{0.32}{\decibel}$. \gls{snr}$_{\text{local}}$ was \SI{32.63}{\decibel} pre-isolation and \SI{9.66}{\decibel} post-isolation, a drop of $\SI{-22.97}{\decibel}$. Figure~\ref{fig:q3-eda-preservation} shows the across-epoch mean conductance and the \gls{psd} for both channels, and the per-epoch Pearson distribution is shown in the right panel of Figure~\ref{fig:q3-pearson-r}. Tables~\ref{tab:q3-isolation-state} and~\ref{tab:q3-isolation-change} summarize the state and change metrics for both modalities.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q3_isolation_eda_preservation.pdf}
  \caption{\gls{eda} amplitude and frequency-content preservation across the isolation stage, Session~2 ($n = \num{30}$). Left: across-epoch mean conductance for the pre-isolation (light blue) and post-isolation (dark blue) channels, extended display window. Right: \gls{psd} for both channels on log-log axes.}
...
\begin{figure}[H]
  ...
  \caption{Per-epoch Pearson correlation between pre- and post-isolation channels. Left: \gls{emg} (Session~1, $n = \num{119}$). Right: \gls{eda} (Session~2, $n = \num{30}$).}
...
\begin{table}[H]
  \centering
  \caption{Isolation stage change metrics for both modalities. Amplitude preservation reported in dB as $20\log_{10}(\text{post} / \text{pre})$. Pearson $r$ reported as mean $\pm$ SD across per-epoch values. \gls{snr} drop computed as post minus pre.}
...
%   q4_emg_psd_four_conditions    P2, Main-Quantitative (2×2 PSD grid)
...
%
% Discussion (DO NOT include here):
%   In-band noise architectural limit (Thread A continuation): §5.3.
%   Notch asymmetry as ISO broadband noise signature: §5.3.

\subsection{Q4 - Digital Filtering}
\label{sec:results-q4}

Figure~\ref{fig:q4-emg-waveform} shows a single contraction epoch from Session~1 across the four conditions: pre-isolation raw, pre-isolation filtered, post-isolation raw, and post-isolation filtered. The post-isolation panels show visibly more baseline noise than the pre-isolation panels.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q4_emg_waveform_single_epoch.pdf}
  \caption{Single-epoch \gls{emg} waveform comparison, Session~1. Top row: pre-isolation raw (left) and pre-isolation filtered (right). Bottom row: post-isolation raw (left) and post-isolation filtered (right). Same epoch index across panels.}
...
Figure~\ref{fig:q4-emg-psd-four} shows the \gls{psd} for each of the four conditions. Integrated power at the \SI{50}{\hertz} mains harmonic was \SI{28.69}{\micro\volt\squared} pre-raw, \SI{0.07}{\micro\volt\squared} pre-filtered, \SI{34.95}{\micro\volt\squared} post-raw, and \SI{0.32}{\micro\volt\squared} post-filtered. Integrated power at the \SI{100}{\hertz} and \SI{150}{\hertz} harmonics was unchanged across the raw and filtered conditions. Integrated notch depth at \SI{50}{\hertz} was \SI{26.18}{\decibel} for the pre-isolation chain and \SI{20.40}{\decibel} for the post-isolation chain.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q4_emg_psd_four_conditions.pdf}
  \caption{\gls{psd} across the four conditions of Session~1: pre-raw (top-left), pre-filtered (top-right), post-raw (bottom-left), post-filtered (bottom-right). Vertical lines mark the \SI{20}{\hertz} \gls{hpf}, \SI{50}{\hertz} notch, and \SI{500}{\hertz} \gls{lpf} cutoffs of the digital chain.}
  \label{fig:q4-emg-psd-four}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Per-condition \gls{emg} metrics, Session~1 ($n = \num{119}$). Mean $\pm$ SD across epochs. Harmonic powers reported as pooled Welch estimates without per-epoch SD.}
  ...
  \end{tabular}
  \\[0.5em]
  {\footnotesize Integrated notch depth at \SI{50}{\hertz}: pre-isolation chain \SI{26.18}{\decibel}, post-isolation chain \SI{20.40}{\decibel}.}
\end{table}
...
% Brief "build" paragraph: introduce the two coupling directions and how
% each is tested. Sessions 3-5 used. Pooling = Method A (per-epoch arrays).
...
%
% Discussion (DO NOT include here):
%   SCR amplitude reproducibility issue (Thread D): §5.3.
%   PSD elevation mechanism candidates (mechanical/rail/cue-arousal/cable): §5.3.
%   Q5 PSD spike pattern at 10-50 Hz harmonic sub-locations (Thread J): §5.3.
%   Q5 P1 dip cross-references Q5b (Thread K): §5.3.
%
...
Figure~\ref{fig:q5-eda-means} shows the across-epoch mean conductance for the three conditions: the Session~2 reference (Section~\ref{sec:results-q2}), the EDA-only block from Sessions 3 to 5 pooled across epochs ($n = \num{30}$), and the Combined block from Sessions 3 to 5 pooled across epochs ($n = \num{30}$). Pooled metrics across Sessions 3 to 5 are computed by combining the per-epoch arrays across sessions (see Section~\ref{sec:q5_emg_effect_on_eda}). A small downward deflection is visible in the Combined trace at approximately $+\SI{5}{\second}$ relative to the \gls{eda} cue, coinciding with the first \gls{emg} cue in the Combined block. Section~\ref{sec:results-q5b} reports the cue-locked analysis. \gls{scl} was $\num{7.714} \pm \num{0.398}$\,\si{\micro\siemens} for the Session~2 reference, $\num{5.923} \pm \num{1.244}$\,\si{\micro\siemens} for the EDA-only block, and $\num{6.334} \pm \num{1.382}$\,\si{\micro\siemens} for the Combined block. The per-session \gls{scl} shift (Combined minus EDA-only) was $+\num{0.332}$\,\si{\micro\siemens} for S3, $+\num{0.228}$\,\si{\micro\siemens} for S4, and $+\num{0.675}$\,\si{\micro\siemens} for S5, with a mean of $+\num{0.412} \pm \num{0.234}$\,\si{\micro\siemens} across the three sessions. Figure~\ref{fig:q5-eda-bars} shows the per-condition \gls{scl} bars.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q5_eda_means_by_condition.pdf}
  \caption{\gls{eda} mean conductance by condition: Session~2 reference ($n = \num{30}$), Sessions 3--5 EDA-only pooled ($n = \num{30}$), Sessions 3--5 Combined pooled ($n = \num{30}$). Display window $-2$ to $+20$\,\si{\second} relative to the \gls{eda} cue. Dashed vertical line marks cue onset.}
  \label{fig:q5-eda-means}
\end{figure}

\gls{scr} amplitude was $\num{0.588} \pm \num{0.158}$\,\si{\micro\siemens} for the Session~2 reference, $\num{0.800} \pm \num{0.211}$\,\si{\micro\siemens} for the EDA-only block, and $\num{0.688} \pm \num{0.180}$\,\si{\micro\siemens} for the Combined block. Per-session values are reported in Section~\ref{sec:results-q7}. Detection rate was \SI{100}{\percent} across all three conditions. \gls{snr}$_{\text{circuit}}$ was \SI{0.60}{\decibel} for the Session~2 reference, $\num{7.53} \pm \num{4.39}$\,\si{\decibel} for the EDA-only block, and $\num{6.19} \pm \num{6.28}$\,\si{\decibel} for the Combined block. \gls{snr}$_{\text{local}}$ was \SI{9.66}{\decibel}, $\num{25.10} \pm \num{5.44}$\,\si{\decibel}, and $\num{22.90} \pm \num{6.20}$\,\si{\decibel} across the same three conditions.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q5_eda_metric_bars.pdf}
  \caption{\gls{eda} scalar metrics by condition. Bars show the pooled mean across S3 to 5 epochs ($n = \num{30}$) for the EDA-only and Combined blocks alongside the Session~2 reference. Error bars show $\pm 1$ SD pooled across epochs. \gls{snr}$_{\text{circuit}}$ and \gls{snr}$_{\text{local}}$ are reported as the across-session mean ($n = 3$ sessions) because per-epoch arrays are not available for these metrics.}
  \label{fig:q5-eda-bars}
\end{figure}

Figure~\ref{fig:q5-eda-psd} shows the \gls{eda} \gls{psd} for the three conditions. The Combined trace exceeds the EDA-only trace across the \SIrange{0.3}{1.3}{\hertz} band and approaches the Session~2 reference level above approximately \SI{1.3}{\hertz}. At \SI{0.5}{\hertz}, \gls{psd} values were $\num{1.57e-04}$\,\si{\micro\siemens\squared\per\hertz} for the Session~2 reference, $\num{1.52e-04}$\,\si{\micro\siemens\squared\per\hertz} for the EDA-only block, and $\num{2.01e-03}$\,\si{\micro\siemens\squared\per\hertz} for the Combined block. At \SI{1.0}{\hertz}, the values were $\num{8.58e-05}$, $\num{2.13e-05}$, and $\num{3.12e-04}$\,\si{\micro\siemens\squared\per\hertz} respectively. Table~\ref{tab:q5-eda-metrics} summarizes the per-condition metrics.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q5_eda_psd_by_condition.pdf}
  \caption{\gls{eda} \gls{psd} by condition. Inset table reports \gls{psd} values at \SI{0.1}{\hertz}, \SI{0.3}{\hertz}, \SI{0.5}{\hertz}, \SI{1.0}{\hertz}, and \SI{1.3}{\hertz} for each condition.}
  \label{fig:q5-eda-psd}
\end{figure}

\begin{table}[H]
  \centering
  \caption{\gls{eda} scalar metrics by condition. Bars show the pooled mean across S3 to S5 epochs ($n = \num{30}$) for the EDA-only and Combined blocks alongside the Session~2 reference. Error bars show $\pm 1$ SD pooled across epochs. \gls{snr}$_{\text{circuit}}$ and \gls{snr}$_{\text{local}}$ are reported as the across-session mean ($n = 3$ sessions), computed as session-level scalars rather than per-epoch arrays.}
...
%
% Discussion (DO NOT include here):
%   Mechanism candidates (mechanical/vascular coupling at palm site): §5.3.
%   Site-specificity of the coupling (palm vs biceps): §5.3 / §5.4.
...
% Framing claim (V2 locked): At the pooled level across S3-5 epochs, EMG
% noise floor and contraction RMS in the Combined block do not exceed the
% Session 1 reference. Pooled Combined-block PSD does not exceed the
% Session 1 reference + 3 dB threshold anywhere in the 0-10 Hz band.
%
% Main evidence:
...
%   q6_emg_psd_baseline_low_freq    P3, Main-Quantitative (0-10 Hz subrange,
%                                    S1 ref + 3 dB threshold annotation)
% Cross-references:
%   Methods metric definitions: \autoref{sec:methods-q6}
%   Per-session breakdown (S4 elevated NF): \autoref{sec:results-q7}
%
% Discussion (DO NOT include here):
%   Why use S1 as reference and not within-session control (Thread L): §5.3.
%   S4 outlier interpretation (Thread F): §5.3.
\subsection{Q6 - EDA Effect on EMG}
\label{sec:results-q6}

The post-isolation \gls{emg} characterization is reported in Section~\ref{sec:results-q1}. Figure~\ref{fig:q6-emg-means} shows the across-epoch envelope mean for the three conditions: the Session~1 reference, the EMG-only block from Sessions 3 to 5 pooled across epochs ($n = \num{120}$), and the Combined block from Sessions 3 to 5 pooled across epochs ($n = \num{120}$). Pooled metrics across Sessions 3 to 5 are computed by combining the per-epoch arrays across sessions (see Section~\ref{sec:q6_eda_effect_on_emg}). Section~\ref{sec:results-q6b} reports the cue-locked analysis of the \gls{emg} signal at \gls{eda} cues.
...
The noise floor was $\num{21.70} \pm \num{0.99}$\,\si{\micro\volt} \gls{rms} for the Session~1 reference, $\num{25.49} \pm \num{13.76}$\,\si{\micro\volt} for the EMG-only block, and $\num{21.49} \pm \num{5.93}$\,\si{\micro\volt} for the Combined block. Contraction \gls{rms} was $\num{32.17} \pm \num{4.98}$\,\si{\micro\volt} for the Session~1 reference, $\num{33.88} \pm \num{13.11}$\,\si{\micro\volt} for the EMG-only block, and $\num{27.85} \pm \num{6.15}$\,\si{\micro\volt} for the Combined block. \gls{snr} was $\num{3.32} \pm \num{1.34}$\,\si{\decibel} for the Session~1 reference, $\num{3.04} \pm \num{1.83}$\,\si{\decibel} for the EMG-only block, and $\num{2.35} \pm \num{1.51}$\,\si{\decibel} for the Combined block. Per-session values are reported in Section~\ref{sec:results-q7}. Figure~\ref{fig:q6-emg-bars} shows the per-condition scalar metric bars.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q6_emg_metric_bars.pdf}
  \caption{\gls{emg} scalar metrics by condition. Bars show the pooled mean across S3 to S5 epochs ($n = \num{120}$) for the EMG-only and Combined blocks alongside the Session~1 reference ($n = \num{119}$). Error bars show $\pm 1$ SD pooled across epochs.}
  \label{fig:q6-emg-bars}
\end{figure}

Figure~\ref{fig:q6-emg-psd-low} shows the \gls{emg} \gls{psd} restricted to the \SIrange{0}{10}{\hertz} subrange for the three conditions, with the Session~1 reference plus \SI{3}{\decibel} threshold marked. The percentage of frequency bins in the \SIrange{0}{10}{\hertz} band exceeding the threshold was \SI{0.0}{\percent} for the EMG-only block and \SI{0.0}{\percent} for the Combined block. Table~\ref{tab:q6-emg-metrics} summarizes the per-condition metrics.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q6_emg_psd_baseline_low_freq.pdf}
  \caption{\gls{emg} \gls{psd} restricted to the \SIrange{0}{10}{\hertz} subrange. Three lines: Session~1 reference, Sessions 3 to 5 EMG-only pooled, Sessions 3 to 5 Combined pooled. Dashed line marks the Session~1 reference $+ \SI{3}{\decibel}$ threshold. Inset reports the percentage of frequency bins for each pooled condition that exceed the threshold.}
  \label{fig:q6-emg-psd-low}
\end{figure}

\begin{table}[H]
  \centering
  \caption{\gls{emg} metrics by condition.Pooled across S3 to S5 epochs ($n = \num{120}$). Mean $\pm$ SD across pooled epochs.}
  \label{tab:q6-emg-metrics}
  \begin{tabular}{l c c c l}
    \toprule
    \textbf{Metric} & \textbf{S1 ref} & \textbf{S3-5 EMG-only} & \textbf{S3-5 Combined} & \textbf{Unit} \\
    \midrule
  ...
  \end{tabular}
  \\[0.5em]
  {\footnotesize Percentage of \SIrange{0}{10}{\hertz} band exceeding Session~1 reference $+ \SI{3}{\decibel}$ threshold: EMG-only \num{0.0}\%, Combined \num{0.0}\%.}
\end{table}
...
%
% Discussion (DO NOT include here):
%   S4 cross-modality asymmetry interpretation (Thread F): §5.3.
%   SCR amplitude reproducibility (Thread D, S4 reverses): §5.3.
%   SNR_circuit cross-session variability (Thread E): §5.3.

\subsection{Q7 - Per-Session Reproducibility}
\label{sec:results-q7}

Figure~\ref{fig:q7-per-session-traces} shows the per-session signal traces across blocks for both modalities. Rows correspond to Sessions 3, 4, and 5. Columns correspond to the \gls{emg} envelope (left) and the \gls{eda} conductance (right). Each panel overlays the single-modality block mean and the Combined block mean.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{result_figures/q7_per_session_traces.pdf}
  \caption{Per-session signal traces across blocks. Rows: Sessions 3, 4, and 5. Columns: \gls{emg} envelope (left) and \gls{eda} conductance (right). Each panel overlays two signal means: solid for the single-modality block and dashed for the Combined block. Line color encodes the session. The y-axis is shared per column.}
...
\begin{figure}[H]
  ...
  \caption{\gls{emg} metrics per session as block trajectories from EMG-only to Combined. Three panels: noise floor, contraction \gls{rms}, and \gls{snr}. Each line connects the two block measurements for the same session.}
...
Figure~\ref{fig:q7-eda-bars} shows the per-session \gls{eda} scalar metrics across both blocks, and Figure~\ref{fig:q7-eda-by-block} shows the same metrics as block trajectories from EDA-only to Combined. \gls{scl} was \SI{6.689}{\micro\siemens} and \SI{7.020}{\micro\siemens} for S3 across the two blocks, \SI{4.412}{\micro\siemens} and \SI{4.640}{\micro\siemens} for S4, and \SI{6.667}{\micro\siemens} and \SI{7.343}{\micro\siemens} for S5. \gls{scr} amplitude was \SI{0.830}{\micro\siemens} and \SI{0.589}{\micro\siemens} for S3, \SI{0.701}{\micro\siemens} and \SI{0.802}{\micro\siemens} for S4, and \SI{0.870}{\micro\siemens} and \SI{0.674}{\micro\siemens} for S5. \gls{snr}$_{\text{circuit}}$ was \SI{7.33}{\decibel} and \SI{4.35}{\decibel} for S3, \SI{12.01}{\decibel} and \SI{13.18}{\decibel} for S4, and \SI{3.24}{\decibel} and \SI{1.03}{\decibel} for S5. \gls{snr}$_{\text{local}}$ was \SI{19.57}{\decibel} and \SI{15.83}{\decibel} for S3, \SI{25.26}{\decibel} and \SI{25.49}{\decibel} for S4, and \SI{30.46}{\decibel} and \SI{27.37}{\decibel} for S5. Table~\ref{tab:q7-eda-metrics} summarizes the per-session per-block \gls{eda} metrics. Session~4 is an outlier on \gls{scl} and \gls{snr}$_{\text{circuit}}$, with values consistently below and above Sessions 3 and 5, respectively, across both blocks.
...
\begin{figure}[H]
  ...
  \caption{\gls{eda} metrics per session as block trajectories from EDA-only to Combined. Four panels: \gls{scl}, \gls{scr} amplitude, \gls{snr}$_{\text{circuit}}$, and \gls{snr}$_{\text{local}}$.}
  \label{fig:q7-eda-by-block}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Per-session per-block \gls{eda} metrics, Sessions 3 to 5. Mean $\pm$ SD across each session-block's epochs ($n = \num{10}$ per block) for \gls{scl} and \gls{scr} amplitude. \gls{snr} values are session-level scalars without per-epoch SD.}
  \label{tab:q7-eda-metrics}
  \begin{tabular}{l l c c c c}
    \toprule
    \textbf{Session} & \textbf{Block} & \textbf{\gls{scl} (\si{\micro\siemens})} & \textbf{\gls{scr} amp (\si{\micro\siemens})} & \textbf{\gls{snr}$_{\text{circuit}}$ (\si{\decibel})} & \textbf{\gls{snr}$_{\text{local}}$ (\si{\decibel})} \\
    \midrule
    S3 & EDA-only & $\num{6.689} \pm \num{0.637}$ & $\num{0.830} \pm \num{0.244}$ & \num{7.33}  & \num{19.57} \\
    S3 & Combined & $\num{7.020} \pm \num{0.688}$ & $\num{0.589} \pm \num{0.149}$ & \num{4.35}  & \num{15.83} \\
    \addlinespace
